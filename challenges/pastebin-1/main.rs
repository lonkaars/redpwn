...
async fn create(mut req: Request<State>) -> tide::Result {
    let Paste { content } = req.body_form().await?; // lees form content
    let id = "abcdefghijklmnopqrstuvwxyz"
        .chars()
        .collect::<Vec<char>>()
        .choose_multiple(&mut rand::thread_rng(), 16)
        .collect::<String>(); // maak willekeurige ID voor de paste
    req.state().value.write().unwrap().insert(id.clone(), content);
    Ok(Redirect::new(format!("/view?id={}", id)).into()) // maak nieuwe url (en return ?????)
}
